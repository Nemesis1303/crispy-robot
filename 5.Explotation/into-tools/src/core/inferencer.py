...
class Inferencer(object):
    ...
    def predict(
        ...
        # Load model
        if (model_for_infer_path / "model.pickle").is_file(): # is bertopic
            tm_model = BERTopicTrainer(model_path=model_for_infer_path.as_posix(), load_trained=True)
        else: # is mallet
            tm_model = MalletLDATrainer(
                model_path=model_for_infer_path.as_posix(),
                mallet_path="/app/Mallet/bin/mallet",
                load_trained=True)
        
        self._logger.info(
            f'-- -- Model loaded from {model_for_infer_path}. Inference STARTS...')
        
        infer_thetas = tm_model.infer(
            docs=texts
        )
